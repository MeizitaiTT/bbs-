							...
							<table class="table table-hover">
								...
								<%for(Subject subject:subjects) {%>
									<tr>
										...
										<td><a href="<%=basePath%>manage/subjectManageTwo.jsp?Id=<%=subject.getId()%>">进入</a>/
										<%if(subject.getStatus()==1){ %>
										<a href="<%=path%>/delete_subject?Id=<%=subject.getId()%>">删除</a>
										<%}else{ %>
										<a href="<%=path%>/recover_subject?Id=<%=subject.getId()%>">回复</a>
										<%} %>
										</td>
